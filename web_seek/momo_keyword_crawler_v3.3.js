// ==UserScript==
// @name         momo 關鍵字搜尋結果抓取器 V3.3（搜尋版+修正特定關鍵字失效）
...
(function () {
  ...
  function onGrabCurrentPage() {
    ...
    try {
      ...
      const stats = grabCurrentPageCore();
      if (!stats) return;
      const { listCount, pageTotal, pageUnique } = stats;
   ...
   * @returns {{listCount:number,pageTotal:number,pageUnique:number}|null}
   */
  function grabCurrentPageCore() {
    ...
    return { listCount: list.length, pageTotal, pageUnique };
  }
...
function extractProductsFromJsonLd() {
  ...
  async function onAutoGrabAcrossPages() {
    ...
    maxExportLimit = target; // 設定「匯出上限 = 最多 N 筆」

    const targetDesc = target > 0 ? `${target} 筆（最多）` : '不限（抓完最後一頁為止）';
    ...
    try {
      ...
      while (isAutoPaging) {
        ...
        if (target > 0 && records.length >= target) {
          if (records.length > target) {
            records.splice(target); // 只保留前 N 筆
          }
          updateCountStatus();
          log(
            `[完成] 已達目標總筆數上限 ${target} 筆，` +
            `實際保留可匯出筆數：${records.length} 筆（多餘部分已截斷），自動跨頁結束。`
          );
          break;
        }
