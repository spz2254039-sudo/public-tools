...
// @version      V2
// @description  匯入 Excel（momo 匯出）建立待輸入清單，自動填寫市場檢查資料、支援自動下一筆、自動分類、自動送出、資料庫紀錄功能。
...
(function () {
  ...
  async function parseExcelFile(file) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const rec = {
        seq: getVal(row, "編號") || String(i),
        mark: getVal(row, "商檢標識"),
        name: getVal(row, "商品名稱"),
        seller_account: getVal(row, "品號"),
        buy_site: getVal(row, "網址/地址"),
        model: "",
        origin: "ZZ",
        status: "符合"
      };
  ...
  function buildPanel() {
    ...
    d.id = "mc-panel";
    d.innerHTML = `
      <div id="mc-header">市場檢查小工具（Excel 版）</div>
      <div id="mc-body">
        <button class="mc-btn" id="mc-btn-importExcel">匯入 Excel（momo 匯出）</button>
        <input type="file" id="mc-file-excel" accept=".xlsx" style="display:none;">
        <div style="margin-top:4px;font-size:11px;color:#555;">
          說明：請直接選擇 momo 原始匯出的 .xlsx 檔案。
        </div>
  ...
  function bindPanelActions() {
    ...
    const importBtn = document.getElementById("mc-btn-importExcel");
