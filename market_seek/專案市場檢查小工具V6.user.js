...
(function () {
  ...
  // V6 變更：
  // 1. 修正 concatUint8 / makeZip 長度與 offset 計算，避免 RangeError: offset is out of bounds。
  // 2. 為 Excel 匯出流程加入 try/catch 與錯誤提示，失敗時自動改用 CSV 匯出。
  // 3. 改善 RAW_ROWS 為空時的提示，提示需先匯入 Excel；匯入後若 RAW_ROWS 不存在會警示重新整理後再匯入。
  ...
  function exportCsvFallback() {
    ...
    const header = "序號,檢查案號,商品名稱,商檢標識,賣家帳號(品號),網址";
  ...
  function bindPanelActions() {
    ...
    document.getElementById("mc-btn-export").onclick = () => {
      ...
      try {
        const header = raw.header && raw.header.length ? raw.header : STANDARD_HEADER.slice();
        const headerLen = header.length;
        const headerIndex = {};
        header.forEach((h, i) => { headerIndex[h] = i; });
        const caseIdx = headerIndex["檢查案號"] !== undefined ? headerIndex["檢查案號"] : 1;

        const rowCaseMap = GM_getValue(ROW_CASE_MAP_KEY, {});
        const rows = (raw.rows || []).map(r => {
          const src = Array.isArray(r.cols) ? r.cols : [];
          const cols = Array(headerLen).fill("");
          for (let i = 0; i < headerLen; i++) {
            cols[i] = src[i] !== undefined && src[i] !== null ? String(src[i]) : "";
          }
          if (r.rowId !== undefined && rowCaseMap[String(r.rowId)] !== undefined) {
            cols[caseIdx] = rowCaseMap[String(r.rowId)];
          }
          return cols;
        });
